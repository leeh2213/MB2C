...
model = ResNetForImageClassification.from_pretrained("/data/lihao/workspace/MB2C/image_models/huggingface/transformers/resnet-50")
model = model.cuda()
model = nn.DataParallel(model, device_ids=[i for i in range(len(gpus))])

processor = AutoImageProcessor.from_pretrained("/data/lihao/workspace/MB2C/image_models/huggingface/transformers/resnet-50")
