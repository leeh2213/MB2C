...
model = CLIPModel.from_pretrained(".cache/huggingface/transformers/clip-vit-large-patch14")
model = model.cuda()
model = nn.DataParallel(model, device_ids=[i for i in range(len(gpus))])

processor = CLIPProcessor.from_pretrained(".cache/huggingface/transformers/clip-vit-large-patch14")
