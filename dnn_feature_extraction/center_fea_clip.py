...
parser.add_argument('--project_dir', default='/data/lihao/workspace/MB2C/Data/Things-EEG2/Image_set', type=str)
...
model = CLIPModel.from_pretrained("/data/lihao/workspace/MB2C/image_models/huggingface/transformers/clip-vit-large-patch14")
model = model.cuda()
model = nn.DataParallel(model, device_ids=[i for i in range(len(gpus))])


processor = CLIPProcessor.from_pretrained("/data/lihao/workspace/MB2C/image_models/huggingface/transformers/clip-vit-large-patch14")
