...
result_path = 'results/' 
model_idx = 'stage2_encoder_training_from_scratch_clip_'
...
class IE():
    ...
    def TrainOneEpoch(self, args, epoch, image_features, eeg_features, labels, test_center, test_center_50, test_eeg, test_eeg_50, labels_a, labels_b, lam,seed_n):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            self.ValOneEpoch(args, epoch, test_center, test_center_50, test_eeg, test_eeg_50,seed_n)
      
    def ValOneEpoch(self, args, epoch, test_center, test_center_50, test_eeg, test_eeg_50,seed_n):
        # validation using test_eeg(test_center brain inference)
        if (epoch + 1) % 1 == 0:
            self.Enc_eeg.eval()
            self.Proj_eeg.eval()
            ...
            with torch.no_grad():
                ...
                for i, (veeg, vimg) in enumerate(self.val_dataloader):
                    ...
                    if total_val_loss <= self.best_loss_val:
                        ...
                        torch.save(self.Enc_eeg.state_dict(), 'model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Enc_eeg_cls.pth')
                        torch.save(self.Proj_eeg.state_dict(), 'model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_eeg_cls.pth')
                        torch.save(self.Proj_img.state_dict(), 'model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_img_cls.pth')
                        torch.save(self.netG.state_dict(), 'model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'netG_cls.pth')
                        torch.save(self.netG2.state_dict(), 'model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'netG2_cls.pth')
        ...
        if not args.reproduce:
            for epoch in tqdm(range(self.n_epochs)):
                ...
                for i, (eeg, img, labels) in tqdm(enumerate(self.dataloader), total=len(self.dataloader), desc=f'Epoch {epoch + 1}/{self.n_epochs}', leave=False):
                    ...
                    if args.is_aug:
                        self.TrainOneEpoch(self.args, epoch, img_features, eeg_features, labels, test_center,test_center_50, test_eeg, test_eeg_50, labels_a, labels_b, lam,seed_n)
                    else:
                        self.TrainOneEpoch(self.args, epoch, img_features, eeg_features, labels, test_center,test_center_50, test_eeg, test_eeg_50,labels, labels, 0.5,seed_n)
...
def main():
    ...
    for j in range(3):
        ...
        # for i in range(5,10):
            ...
            print('Subject %d' % (i+1))
            ie = IE(args, i + 1,seed_n)
            print('MixRatio:',args.MixRatio)
            starttime = datetime.datetime.now()

            Acc, Acc3, Acc5, Acc_50, Acc3_50, Acc5_50 = ie.train(args, seed_n)
