...
from utils.augment import augment_fuc
...
model_idx = 'leave_one_subject_stage2_encoder_clip_mixup0.75'
...
parser.add_argument('--cyclelambda', type=float, default=1)
...
class IE():
    ...
    def get_eeg_data(self):
        ...
        for i in range(1,11):
            if i==self.nSub:
                ...
                data = np.load(self.eeg_data_path + '/sub-' + format(i, '02') + '/preprocessed_eeg_training.npy', allow_pickle=True)
                # (16540, 4, 63, 250)
    ...
    def get_image_data(self):
        ...
        train_img_feature = np.tile(train_img_feature, (9, 1, 1))
    ...
    def TrainOneEpoch(self, args, epoch, image_features, eeg_features, labels, test_center, test_eeg, labels_a, labels_b, lam,seed_n):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            self.ValOneEpoch(args, epoch, test_center, test_eeg,seed_n)
      
    def ValOneEpoch(self, args, epoch, test_center, test_eeg,seed_n):
        # validation using test_eeg(test_center brain inference)
        if (epoch + 1) % 1 == 0:
            self.Enc_eeg.eval()
            self.Proj_eeg.eval()
            ...
            with torch.no_grad():
                ...
                for i, (veeg, vimg) in enumerate(self.val_dataloader):
                    ...
                    if total_val_loss <= self.best_loss_val:
                        ...
                        self.log_write.write('Epoch %d: , top1_200: %.4f ,top3_200: %.4f ,top5_200: %.4f, seed_n:%.4f\n'%((epoch + 1), top1 ,top3,top5,seed_n))
    ...
    def train(self, args, seed_n):
        ...
        train_label = np.repeat(np.arange(1654), 10)
        train_label = torch.from_numpy(np.tile(train_label,(9)))
        ...
        val_eeg = torch.from_numpy(train_eeg[:6660])
        val_image = torch.from_numpy(train_img_feature[:6660])

        train_eeg = torch.from_numpy(train_eeg[6660:])
        train_image = torch.from_numpy(train_img_feature[6660:])
        train_label = train_label[6660:]
        ...
        if not args.reproduce:
            for epoch in tqdm(range(self.n_epochs)):
                ...
                for i, (eeg, img, labels) in tqdm(enumerate(self.dataloader), total=len(self.dataloader), desc=f'Epoch {epoch + 1}/{self.n_epochs}', leave=False):
                    ...
                    if args.is_aug:
                        self.TrainOneEpoch(self.args, epoch, img_features, eeg_features, labels, test_center, test_eeg, labels_a, labels_b, lam,self.seed)
                    else:
                        self.TrainOneEpoch(self.args, epoch, img_features, eeg_features, labels, test_center, test_eeg, labels, labels, 0.5,self.seed)
...
def main():
    ...
    column.append('ave'+'_'+ str(seed_n))
