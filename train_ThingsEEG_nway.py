...
from util.end_to_end_encoding_utils import *
from Loss.ganModelCls import _param,_netG,_netD,_netG2,_netD2
from Loss.gradient_penalty import GradientPenalty
...
result_path = '/data/lihao/workspace/MB2C/results/' 
os.makedirs(result_path, exist_ok=True)
model_idx = 'encoder_training_from_scratch_clip_'

class MB2C():
    def __init__(self, args, nsub, nseed):
        super(MB2C, self).__init__()
        self.args = args
        self.num_class = 200
        ...
        self.start_epoch = 0
        self.eeg_data_path = '/data/lihao/workspace/MB2C/Data/Things-EEG2/Preprocessed_data_250Hz/'
        self.img_data_path = '/data/lihao/workspace/MB2C/Data/Things-EEG2/DNN_feature_maps/pca_feature_maps/' + args.dnn + '/pretrained-True/'
        self.test_center_path = '/data/lihao/workspace/MB2C/Data/Things-EEG2/Image_set/'
        self.save_ckpt_path = '/data/lihao/workspace/MB2C/model/'
        ...
        if args.pretrained:
            self.Enc_eeg.load_state_dict(torch.load('model/' + model_idx + str(self.nseed) +'_'+ str(self.nSub) + 'Enc_eeg_cls.pth'))
            self.Proj_eeg.load_state_dict(torch.load('model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_eeg_cls.pth'))
            self.Proj_img.load_state_dict(torch.load('model/' + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_img_cls.pth'))
        ##################################################################################################
    ...
    def TrainOneEpoch(self, args, epoch, image_features, eeg_features, labels, test_center, test_center_50, test_eeg, test_eeg_50, labels_a, labels_b, lam):
        with torch.autograd.set_detect_anomaly(True):
            ...
            if args.is_gan:
                ...
                for _ in range(1):
                    ...
                    cycle_loss1 = self.args.cyclelambda * torch.nn.L1Loss()(eeg_feat, back_eeg_sample)
                    cycle_loss2 = self.args.cyclelambda * torch.nn.L1Loss()(X, back_img_sample)
            ...
            self.ValOneEpoch(args, epoch, test_center, test_center_50, test_eeg, test_eeg_50)
      
    def ValOneEpoch(self, args, epoch, test_center, test_center_50, test_eeg, test_eeg_50):
        if (epoch + 1) % 1 == 0:
            self.Enc_eeg.eval()
            self.Proj_eeg.eval()
            self.Proj_img.eval()
            ...
            with torch.no_grad():
                ...
                for i, (veeg, vimg) in enumerate(self.val_dataloader):
                    ...
                    if args.is_gan:
                        ...
                        cycle_loss1 = self.args.cyclelambda * self.criterion_l1(veeg_feat, back_eeg_sample)
                        cycle_loss2 = self.args.cyclelambda * self.criterion_l1(vimg_features, back_img_sample)
                        ...
                        self.writer.add_scalars("Total Validation Loss", {"total_val_loss":cycle_loss,"cycle_loss":cycle_loss,"infoNCEloss":vloss_cos}, global_step=epoch)
                        
                    if total_val_loss <= self.best_loss_val:
                        self.best_loss_val = total_val_loss
                        self.best_epoch = epoch + 1
                        torch.save(self.Enc_eeg.state_dict(), self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Enc_eeg_cls.pth')
                        torch.save(self.Proj_eeg.state_dict(), self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_eeg_cls.pth')
                        torch.save(self.Proj_img.state_dict(), self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_img_cls.pth')
                        torch.save(self.netG.state_dict(), self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'netG_cls.pth')
                        torch.save(self.netG2.state_dict(), self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'netG2_cls.pth')
                        print('Best model saved!')
                        
                        proj_test_center = self.Proj_img(test_center)
                        norm_test_center = proj_test_center / proj_test_center.norm(dim=1, keepdim=True)
                        top1, top3, top5 = run_classification_test(args, norm_test_center, self.Proj_eeg(self.Enc_eeg(Variable(test_eeg.type(self.Tensor)))), (epoch+1))
                        self.log_write.write('Epoch %d: , top1_200: %.4f ,top3_200: %.4f ,top5_200: %.4f, seed_n:\n'%((epoch + 1), top1 ,top3,top5))
                        ...
                        top1_50, top3_50, top5_50 = run_classification_test(args, norm_test_center_50, self.Proj_eeg(self.Enc_eeg(Variable(test_eeg_50.type(self.Tensor)))), (epoch+1))
                        self.log_write.write('Epoch %d: , top1_50: %.4f ,top3_50: %.4f ,top5_50: %.4f, seed_n:\n'%((epoch + 1), top1_50 ,top3_50, top5_50))
    ...
    def train(self, args):
        ...
        train_eeg, train_label, test_eeg, test_label = self.get_eeg_data()
        # (16540, 768)
        ...
        if not args.reproduce:
            for epoch in tqdm(range(self.n_epochs)):
                ...
                for i, (eeg, img, labels) in tqdm(enumerate(self.dataloader), total=len(self.dataloader), desc=f'Epoch {epoch + 1}/{self.n_epochs}', leave=False):
                    ...
                    if args.is_aug:
                        self.TrainOneEpoch(self.args, epoch, img_features, eeg_features, labels, test_center,test_center_50, test_eeg, test_eeg_50, labels_a, labels_b, lam)
                    else:
                        self.TrainOneEpoch(self.args, epoch, img_features, eeg_features, labels, test_center,test_center_50, test_eeg, test_eeg_50,labels, labels, 0.5)
        ...
        # * test part
        self.Enc_eeg.load_state_dict(torch.load(self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Enc_eeg_cls.pth'))
        self.Proj_eeg.load_state_dict(torch.load(self.save_ckpt_path+ model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_eeg_cls.pth'))
        self.Proj_img.load_state_dict(torch.load(self.save_ckpt_path + model_idx + str(self.nseed) +'_'+str(self.nSub) + 'Proj_img_cls.pth'))
        ...
        with torch.no_grad():
            ...
            proj_test_center_50 = self.Proj_img(test_center_50)
            norm_test_center_50 = proj_test_center_50 / proj_test_center_50.norm(dim=1, keepdim=True)
            top1_acc_50, top3_acc_50, top5_acc_50 = run_classification_test(args, norm_test_center_50, eeg_feat_50, 0)
...
def main():
    ...
    for i in range(1, 2):
        ...
        print('Subject %d' % (i+1))
        mb2c = MB2C(args, i + 1, seed_n)
        print('MixRatio:',args.MixRatio)
        starttime = datetime.datetime.now()

        Acc, Acc3, Acc5, Acc_50, Acc3_50, Acc5_50 = mb2c.train(args)
    ...
    print('aver  aver3  aver5  aver_50  aver3_50 aver5_50')
    print([aver, aver3, aver5, aver_50, aver3_50, aver5_50])
    # column = np.arange(1, cal_num).tolist()
    # # column.append('ave'+str(j)+'_'+str(seed_n))
