...
def image_inference(args,image_model, image_test):
    image_test=image_test.to(args.device)
    with torch.no_grad():
        image_model.eval()
        results = image_model(image_test)
        return results.cpu().numpy()

def brain_inference(args,eeg_model,EEG_test):
